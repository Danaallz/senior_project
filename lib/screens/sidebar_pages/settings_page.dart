...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> loadProfile() async {
    ...
    if (profile != null) {
      ...
      _phoneController.text =
          savedPhone == null || savedPhone.toString() == '0'
              ? ''
              : savedPhone.toString();
      ...
      selectedPhoneCode = profile['phone_code'] ?? '+966';
    }
  }
  ...
  Future<void> pickAndUploadImage() async {
    ...
    try {
      selectedProfileImageBytes = await image.readAsBytes();

      if (selectedProfileImageBytes!.length > 5 * 1024 * 1024) {
        if (!mounted) return;

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Image size must be less than 5 MB."),
            backgroundColor: Colors.red,
          ),
        );
  ...
  Future<void> saveSettings() async {
    ...
    try {
      await supabaseService.updateProfile(
        profileId: profileId!,
        data: {
          'company_name': cleanInput(_companyController.text),

          'phone_code': selectedPhoneCode,

          'phone': cleanInput(_phoneController.text),

          'address': cleanInput(_addressController.text),

          'country': selectedCountry,

          'state': cleanInput(_stateController.text),

          'city': cleanInput(_cityController.text),

          'zip_code': cleanInput(_zipController.text),
        },
      );
                    ...
                    Expanded(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            title: Text(country),
                            trailing: Text(countryPhoneCodes[country] ?? ''),
                            onTap: () {
                              setState(() {
  ...
  String? validateZip(String? value) {
    ...
    if (!RegExp(r'^[A-Za-z0-9 -]{3,12}$').hasMatch(text)) {
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: const OutlineInputBorder(
              borderSide: BorderSide(color: Color(0xFF0D1A3A)),
            ),
  ...
  Widget _buildCountryPicker() {
    return Column(
      ...
      children: [
        ...
        GestureDetector(
          onTap: loadingCountries ? null : showCountrySearchDialog,
          child: Container(
            height: 56,
  ...
  Widget _buildPhoneField() {
    final phoneCodes = countryPhoneCodes.values.toSet().toList()..sort();
          ...
          children: [
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                value:
                    phoneCodes.contains(selectedPhoneCode)
                        ? selectedPhoneCode
                        : null,
